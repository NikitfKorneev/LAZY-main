...
if (isset($_GET["ID"])) { //Если запрос от устройства содержит идентификатор
    $result = mysqli_query($mysql, "SELECT * FROM DEVICE_TABLE WHERE DEVICE_ID='" . $_GET['ID'] . "'"); // запрос к БД
    if (mysqli_num_rows($result) == 1) { //Если найдено устройство с таким ID в БД
        if (isset($_GET['Rele'])) { //Если устройство передало новое состояние реле
            $result = mysqli_query($mysql, "SELECT OUT_STATE FROM OUT_STATE_TABLE WHERE DEVICE_ID = '" . $_GET['ID'] . "'");   //проверяем есть ли в БД предыдущее значение этого параметра
...
//--------------------------Температура-----------------------
        ...
        if (isset($_GET['Term'])) { //Если устройство передало новое значение температуры
            $result = mysqli_query($mysql, "SELECT TEMPERATURE FROM TEMPERATURE_TABLE WHERE DEVICE_ID='" . $_GET['ID'] . "'"); //проверяем есть ли в БД предыдущее значение этого параметра
            $date_today = date("Y-m-d H:i:s"); //текущее время
            if (mysqli_num_rows($result) == 1) { //Если в таблице есть данные для этого устройства - обновляем
